...
def main():
    bundle = joblib.load("ids_model_v3.pkl")
    model = bundle["model"]
    label_encoder = bundle["label_encoder"]
    feature_columns = bundle["feature_columns"]

    # --- 1. Internal held-out split (same source file as training) ---
    X_train_file, y_train_file = load_and_prepare_data("KDDTrain+_20Percent.txt")
    ...
    try:
        X_ext, y_ext = load_and_prepare_data("KDDTest+.txt")
        ...
        print("[!] 'KDDTest+.txt' not found - skipping external evaluation. "
              "Download it and place it in this folder to see real generalization performance.")
